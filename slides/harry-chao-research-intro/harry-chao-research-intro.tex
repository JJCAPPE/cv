...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture,overlay]
    \node[anchor=center, opacity=0.14] at (current page.center) {\includegraphics[width=\paperwidth]{rowing_pose_overlay.png}};
  ...
  \end{tikzpicture}
  \source{Visual: rowing video-to-pose artifact from local research journal/repo.}
...
\begin{frame}[plain]
  ...
  \begin{tikzpicture}[x=1cm,y=1cm]
    ...
    \foreach \x/\c/\t/\s in {
      ...
      3.2/blue/{2024}/{MNIST from scratch},
    ...
    \node[draw=line, fill=white, rounded corners=3pt, inner sep=3pt] at (3.2,0.35) {\includegraphics[width=2.0cm]{digit_training.png}};
    \node[draw=line, fill=white, rounded corners=3pt, inner sep=6pt] at (5.5,0.35) {\includegraphics[width=0.85cm,interpolate=false]{cifar_sample.png}};
    \node[draw=line, fill=white, rounded corners=3pt, inner sep=3pt] at (10.3,0.35) {\includegraphics[width=2.05cm]{rowing_force_curves.png}};
  \end{tikzpicture}
  ...
  \end{center}
  \source{Project trail: digit-recognizer, CIFAR\_10, NoteWorthy, rowing-dynamics-analysis.}
...
% Slide 3
\begin{frame}[plain]
  \slidetitle{Current research: video-to-force inference}
  \begin{columns}[T,totalwidth=\textwidth]
    ...
    \begin{column}{0.48\textwidth}
      \includegraphics[width=\linewidth]{rowing_force_curves.png}
      \vspace{0.15cm}
  ...
  \end{center}
  \source{Independent Presidential Scholarship project; local journal.pdf and rowing-dynamics-analysis repo.}
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture,overlay]
    \node[anchor=center, opacity=0.16] at (current page.center) {\includegraphics[width=1.05\paperwidth]{rowing_matching_diagnostic.png}};
  ...
  \end{center}
  \source{Fit to Chao research themes: data, evaluation, algorithms; imperfect/limited/noisy data.}
...
\begin{frame}[plain]
  ...
  \begin{columns}[T,totalwidth=\textwidth]
    \begin{column}{0.46\textwidth}
      {\small\bfseries Chao group themes}
  ...
  \vfill
  \begin{center}
    {\Large\bfseries A small real-world CV system with limited supervision is the bridge.}
  \end{center}
  \source{Sources: Chao BU/HIC profiles; Chao lab research page and publications by topic.}
...
\begin{frame}[plain]
  ...
  \vfill
  \begin{tikzpicture}
    \node[draw=line, fill=white, rounded corners=3pt, text width=11.8cm, align=center, inner sep=7pt, font=\scriptsize] {
      Relevant courses: Probability / Statistics / Data; Computational Linear Algebra; Applied Algorithms; Logic Design; Software Engineering; Multivariable Calculus; Differential Equations; Physics; CS I/II credit.
    };
  \end{tikzpicture}
  \source{Course list from BU transcript; availability from June email to Prof. Chao.}
